...
- **Status:** v1 delivered — PHASE-01..06 closed (epic `python-foundry-cli-0b8`,
  exit milestone **MS-006**). See `docs/evidence/` for MS-001..MS-006 evidence.
- All commands are live: `validate`, `plan`, `generate`, `catalog list/show`,
  `version`. Stage/place, real `generate` + verify + lock, full catalog
  content (cli/scripts/data-etl archetypes; http/hooks-hk/data-etl profiles),
  and the hybrid GitHub template are all implemented and tested.
- Run `bd list --status=closed` / `bd show python-foundry-cli-0b8` for the
  full delivery graph; do not re-derive phase status from this file alone.
...
src/python_foundry/
  ...
  render/    # stage rendering (template + static)
  fsx/       # stage + exclusive place (PHASE-02)
  generate/  # orchestrate + uv.lock production (PHASE-03)
  verify/    # verify tier runners (PHASE-03)
